...
var FAKE_OUTLET = {
    ...
    console.log("Turning the beamer %s!...", on ? "on" : "off");
    if (on) {
          FAKE_OUTLET.powerOn = true;
          if(err) { return console.log(err); }
          eventEmitter.emit('beamerOn');
          console.log("...beamer is now on.");
    } else {
          FAKE_OUTLET.powerOn = false;
          if(err) { return console.log(err); }
          eventEmitter.emit('beamerOff');
          console.log("...beamer is now off.");
    }
  },
    identify: function() {
...
var outletUUID = uuid.generate('hap-nodejs:accessories:BOutlet');

// This is the Accessory that we'll return to HAP-NodeJS that represents our fake light.
var outlet = exports.accessory = new Accessory('Beamer', outletUUID);

// Add properties for publishing (in case we're using Core.js and not BridgedCore.js)
outlet.username = "1A:2B:3C:4D:5D:F9";
